...
/**
 * Custom hook to get real-time message count (received from admin only)
...
export const useMessageCount = () => {
  ...
  const fetchMessageCount = useCallback(async () => {
    ...
    try {
      ...
      // Filter: Only count messages received from admin
      const allMessages = response.data || [];
      const receivedFromAdminCount = allMessages.filter((msg) => {
        return (
      ...
      setMessageCount(receivedFromAdminCount);
    } catch (err) {
      console.error('[useMessageCount] Error fetching message count:', err);
