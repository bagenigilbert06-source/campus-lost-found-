            ...
            {recoverItem.map((item) => (
              <div key={item._id} className="border p-4 rounded-md bg-gray-50 shadow-sm hover:bg-gray-100">
                ...
                <div className="mt-4 text-sm text-gray-500 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <FiCalendar />
                    <span>{(item.recoveredDate || item.dateLost || item.updatedAt)
                      ? new Date(item.recoveredDate || item.dateLost || item.updatedAt).toLocaleDateString()
                      : 'Unknown date'
                    }</span>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              <tbody className="text-gray-800">
                {recoverItem.map((item) => (
                  <tr key={item._id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="border px-4 py-2">{(item.recoveredDate || item.dateLost || item.updatedAt)
                      ? new Date(item.recoveredDate || item.dateLost || item.updatedAt).toLocaleDateString()
                      : 'Unknown date'
                    }</td>
