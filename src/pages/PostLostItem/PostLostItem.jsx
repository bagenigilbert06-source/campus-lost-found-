  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.status === 201 || response.data?.success || response.data?._id) {
        await Swal.fire({
          title: "Success!",
          text: "Your lost item has been posted successfully. Admin will review it shortly.",
          icon: "success",
      ...
      toast.error(
        error?.response?.data?.message || "Error posting lost item. Please try again."
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="rounded-2xl border border-base-300 bg-base-200/60 p-4">
          <label className="flex cursor-pointer items-start gap-3">
            ...
            <span className="text-sm leading-6 text-base-content/80">
              I confirm that the information provided is accurate and true. The
              admin will verify this item before posting it publicly.
            </span>
          </label>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-6">
          <h1 className="text-3xl font-bold tracking-tight md:text-4xl">
            Report a {formData.postType || 'Lost'} Item
          </h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-base-content/70 md:text-base">
            Fill in the details below so the item can be reviewed and matched
            more easily.
          </p>
        </div>
              ...
              {currentStep < steps.length ? (
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <FaCheckCircle />
                      Post {formData.postType || 'Lost'} Item
                    </>
                  )}
                </button>
              )}
